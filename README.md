...
29. SHEL.L, AZN.L, ULVR.L stay fail-closed with distinct deny `LSE_SESSION_RULE_UNSET` until the Board picks a session rule (Addendum C flatten-at-US-close vs LSE cash end 16:30). They cannot silently fill at Grand Opening. US names still wait on Hari Grand Opening PAPER. Do not invent US listings. Do not rewrite Addendum C. Employees cannot write this lock. PAPER execution stays CLOSED.
