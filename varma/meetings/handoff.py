"""Meeting handoff artefacts. Database is source of truth (Documents 09, 18)."""
...
def get_ceo(session: Session) -> Employee:
    ...
    if ceo is None:
        raise RuntimeError("CEO employee missing — seed_if_empty must persist the CEO identity")
    return ceo
...
def handoff_brief_to_ceo(
    ...
    MemoryStores(session).working_put(ceo.id, "last_handoff_id", row.id)
    ceo.status = "AVAILABLE"
    ceo.status_bubble = "PACK READY"
    session.commit()
    return row
